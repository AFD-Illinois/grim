                               ...
                               void *ptr)
{
  ...
  {
    Vec primPetscVecOldLocal, primPetscVecHalfStep;
    DMGetLocalVector(ts->dmdaWithGhostZones, &primPetscVecOldLocal);
    DMGetLocalVector(ts->dmdaWithGhostZones, &primPetscVecHalfStep);
...
#pragma ivdep
      ...
      /* Work on the tiles.*/
      computeFluxesOverTile(primTile, fluxX1Tile, fluxX2Tile);
      ...
      LOOP_INSIDE_TILE(0, TILE_SIZE_X2, 0, TILE_SIZE_X1)
      {
        ...
        for (int var=0; var<DOF; var++)
        {
          ...
          divFluxGlobal[INDEX_GLOBAL(zone, var)] = 
    ...
    DMRestoreLocalVector(ts->dmdaWithGhostZones, &primPetscVecHalfStep);
